...
export class RoboticWorkforce extends Card implements IProjectCard {
  ...
  public play(player: Player, game: Game) {
    ...
    return new SelectCard('Select builder card to copy', 'Copy', availableCards, (selectedCards: Array<ICard>) => {
      ...
      class Updater {
        public productionUnits: Units;
        constructor(
          public name: CardName,
          public partialProduction: PartialUnits,
        ) {
          this.productionUnits = Units.of(partialProduction);
        };
      }
      ...
      const updaters: Array<Updater> = [
        // AI Central isn't even needed anymore, because it's stored on the card.
        new Updater(CardName.BIOFERTILIZER_FACILITY, {plants: 1}),
        new Updater(CardName.BUILDING_INDUSTRIES, {energy: -1, steel: 2}), // -1, 0, 2, 0, 0, 0),
        new Updater(CardName.CAPITAL, {energy: -2, megacredits: 5}), // -2, 5, 0, 0, 0, 0),
        new Updater(CardName.CAPITAL_ARES, {energy: -2, megacredits: 5}), // -2, 5, 0, 0, 0, 0),
        ...
        // new Updater(CardName.PARLIAMENT_HALL, 0, Math.floor(player.getTagCount(Tags.BUILDING) / 3), 0, 0, 0, 0),
      ...
      } else if (staticCardProperties.get(foundCard.name)) {
        const prop = staticCardProperties.get(foundCard.name);
        if (prop?.productionBox !== undefined) {
          const units = Units.of(prop.productionBox);
          result = new Updater(
            foundCard.name,
            units,
          );
      ...
      if (!player.hasProduction(result.productionUnits)) {
      ...
      player.adjustProductionUnits(result.productionUnits);
