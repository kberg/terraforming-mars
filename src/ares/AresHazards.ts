...
export class _AresHazardPlacement {
  ...
  public static randomlyPlaceHazard(game: Game, tileType: TileType, direction: 1 | -1, raiseError: boolean = true) {
    const space = game.getSpaceByOffset(direction, tileType, raiseError);
  ...
  private static testToPlaceErosionTiles(aresData: IAresData, player: Player) {
    ...
    this.testConstraint(
      ...
      player.game.board.getOceansOnBoard(player.game.gameOptions.automaSoloVariant),
      () => {
