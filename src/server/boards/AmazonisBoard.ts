...
export class AmazonisBoard extends MarsBoard {
  public static newInstance(gameOptions: GameOptions, rng: Random): AmazonisBoard {
    const builder = new BoardBuilder(AmazonisBoard, gameOptions, rng);
    ...
    // y=1
    builder.ocean(TITANIUM).volcanic(MICROBE, MICROBE).land().land().ocean(DRAW_CARD, DRAW_CARD).ocean();
    // y=2
    ...
    // y=4
    builder.land(PLANT).land(PLANT).land().land(HEAT, HEAT).restricted().unshufflable()
      .land(HEAT, HEAT).volcanic(PLANT, PLANT).land().land(TITANIUM, TITANIUM);
    // y=5
    builder.ocean(PLANT, PLANT).land(PLANT).land(STEEL).land(HEAT, PLANT).land(PLANT).volcanic(DRAW_CARD).land().ocean(PLANT);
    // y=6
    builder.ocean(PLANT).land().land(MICROBE).volcanic(HEAT, PLANT).land().land(PLANT, PLANT).ocean(PLANT, PLANT);
    // y=7
