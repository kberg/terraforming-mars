...
export class TurmoilHandler {
  ...
  public static computeTerraformRatingBump(player: IPlayer, tr: TRSource = {}): number {
    ...
    if (tr.oxygen !== undefined) {
      const availableSteps = player.game.getMaxOxygen() - player.game.getOxygenLevel();
    ...
    if (tr.temperature !== undefined) {
      const availableSteps = Math.floor((player.game.getMaxTemperature() - player.game.getTemperature()) / 2);
    ...
    if (tr.oceans !== undefined) {
      const availableSteps = player.game.getMaxOceans() - player.game.board.getOceanSpaces().length;
      const steps = Math.min(availableSteps, tr.oceans);
      total = total + steps;
    }

    if (tr.venus !== undefined) {
      const availableSteps = Math.floor((player.game.getMaxVenus() - player.game.getVenusScaleLevel()) / 2);
