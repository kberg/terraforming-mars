...
export class AsteroidMining implements IGlobalEvent {
    ...
    public resolve(game: Game, turmoil: Turmoil) {
      game.getPlayers().forEach((player) => {
        player.setResource(Resources.TITANIUM, Math.min(5, player.getTagCount(Tags.JOVIAN, false, false)) + turmoil.getPlayerInfluence(player), game, undefined, GlobalEventName.ASTEROID_MINING);
      });
    }
}
