    ...
    public resolve(game: Game, turmoil: Turmoil) {
      game.getPlayers().forEach((player) => {
        ...
        if (player.getProduction(Resources.ENERGY) >= 1) {
          player.addProduction(Resources.ENERGY, -1, game, undefined, GlobalEventName.SABOTAGE);
        }
        if (player.getProduction(Resources.STEEL) >= 1) {
          player.addProduction(Resources.STEEL, -1, game, undefined, GlobalEventName.SABOTAGE);
        }
        player.setResource(Resources.STEEL, turmoil.getPlayerInfluence(player), game, undefined, GlobalEventName.SABOTAGE);
      });
    }
}
