...
export class SpinoffProducts implements IGlobalEvent {
    ...
    public resolve(game: Game, turmoil: Turmoil) {
      game.getPlayers().forEach((player) => {
        player.setResource(Resources.MEGACREDITS, 2 * (Math.min(5, player.getTagCount(Tags.SCIENCE, false, false)) + turmoil.getPlayerInfluence(player)), game, undefined, GlobalEventName.SPINOFF_PRODUCTS);
      });
    }
}
