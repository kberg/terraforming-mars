    ...
    public resolve(game: Game, turmoil: Turmoil) {
      game.getPlayers().forEach((player) => {
        if (player.getResource(Resources.HEAT) > 0) {
          player.deductHeat(player.getResource(Resources.HEAT), {globalEvent: true});
        }
        const maxedSteelTags = Math.min(5, player.getTagCount(Tags.BUILDING, false, false));
        player.deductMegacredits(2 * Math.max(0, maxedSteelTags - turmoil.getPlayerInfluence(player)), {globalEvent: true});
      });
    }
}
