    ...
    public resolve(game: Game, turmoil: Turmoil) {
      game.getPlayers().forEach((player) => {
        if (player.getResource(Resources.HEAT) > 0) {
          player.setResource(Resources.HEAT, -player.getResource(Resources.HEAT), game, undefined, GlobalEventName.GLOBAL_DUST_STORM);
        }
        const maxedSteelTags = Math.min(5, player.getTagCount(Tags.BUILDING, false, false));
        player.setResource(Resources.MEGACREDITS, -2 * Math.max(0, maxedSteelTags - turmoil.getPlayerInfluence(player)), game, undefined, GlobalEventName.GLOBAL_DUST_STORM);
      });
    }
}
