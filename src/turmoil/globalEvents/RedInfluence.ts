...
export class RedInfluence implements IGlobalEvent {
    ...
    public resolve(game: Game, turmoil: Turmoil) {
      game.getPlayers().forEach((player) => {
        ...
        if (amount > 0) {
          player.setResource(Resources.MEGACREDITS, amount * -3, game, undefined, GlobalEventName.RED_INFLUENCE);
        }
        player.addProduction(Resources.MEGACREDITS, turmoil.getPlayerInfluence(player), game, undefined, GlobalEventName.RED_INFLUENCE);
      });
    }
}
