...
export class Productivity implements IGlobalEvent {
    ...
    public resolve(game: Game, turmoil: Turmoil) {
      game.getPlayers().forEach((player) => {
        player.setResource(Resources.STEEL, Math.min(5, player.getProduction(Resources.STEEL)) + turmoil.getPlayerInfluence(player), game, undefined, GlobalEventName.PRODUCTIVITY);
      });
    }
}
