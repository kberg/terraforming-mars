...
export class ImprovedEnergyTemplates implements IGlobalEvent {
    ...
    public resolve(game: Game, turmoil: Turmoil) {
      game.getPlayers().forEach((player) => {
        player.addProduction(Resources.ENERGY, Math.floor((player.getTagCount(Tags.ENERGY, false, false) + turmoil.getPlayerInfluence(player)) / 2), game, undefined, GlobalEventName.IMPROVED_ENERGY_TEMPLATES);
      });
    }
}
