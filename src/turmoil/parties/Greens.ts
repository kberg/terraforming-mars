...
class GreensBonus01 implements Bonus {
  ...
  getScore(player: Player) {
    return player.getTagCount(Tags.PLANT, 'raw') + player.getTagCount(Tags.MICROBE, 'raw') + player.getTagCount(Tags.ANIMAL, 'raw');
  }
...
class GreensPolicy04 implements Policy {
  ...
  description: string = 'Spend 5 M€ to gain 3 plants or add 2 microbes to ANY card';
  isDefault = false;
  ...
  action(player: Player, isDominantPartyAction: boolean = false) {
    ...
    game.log('${0} used Turmoil ${1} action', (b) => b.player(player).card(new GreensPolicy4Action()));
