...
describe('LogHelper', function() {
  it('logs drawn cards by card', function() {
    ...
    expect(msg!.message).eq('${0} drew no cards');
    LogHelper.logDrawnCards(player1, [card1]);
    msg = game.gameLog.pop()!;
    expect(msg.data.length).eq(2);
    expect(msg.data[0].value).eq(player1.color);
    expect(msg.data[1].value).eq(card1.name);
    expect(msg.message).eq('${0} drew ${1}');
    LogHelper.logDrawnCards(player1, [card1, card2]);
    msg = game.gameLog.pop()!;
    expect(msg.data.length).eq(3);
    expect(msg.data[0].value).eq(player1.color);
    expect(msg.data[1].value).eq(card1.name);
    expect(msg.data[2].value).eq(card2.name);
    expect(msg.message).eq('${0} drew ${1} and ${2}');
    LogHelper.logDrawnCards(player1, [card1, card2, card3]);
    msg = game.gameLog.pop()!;
    expect(msg.data.length).eq(4);
    expect(msg.data[0].value).eq(player1.color);
    expect(msg.data[1].value).eq(card1.name);
    expect(msg.data[2].value).eq(card2.name);
    expect(msg.data[3].value).eq(card3.name);
    expect(msg.message).eq('${0} drew ${1}, ${2} and ${3}');
  });
