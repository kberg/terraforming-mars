...
describe('Herbivores', () => {
  ...
  it('Can not play if nobody has plant production', () => {
    ...
    expect(player.canPlay(card, {testAffordability: false}, {testAffordability: false})).is.not.true;
  });
  ...
  it('Should play - auto select if single target', () => {
    setOxygenLevel(game, 8);
    player2.production.add(Resource.PLANTS, 1);
    expect(player.canPlay(card, {testAffordability: false}, {testAffordability: false})).is.true;
